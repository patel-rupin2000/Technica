...
export default function AuthScreen (props){
  //AsyncStorage.clear().then(() => console.log('Cleared'))
	...
	const signUpHandler =  async(email, password) =>{
        setIsLoading(true)
        const response  =  await fetch("https://identitytoolkit.googleapis.com/v1/accounts:signUp?key= AIzaSyCSniPVb9P0B27ENWjQzDQim3T7Z-trAUs",{
			method :'POST',
			headers:{
        ...
        const response2  =  await fetch("https://tech-cc0c4.firebaseio.com/users.json",{
			method :'POST',
			headers:{
				'Content-Type':'application/json'
			},
	...
	const loginHandler =   async (email, password) =>{
        ...
        setIsLoading(true)
        const response  =  await fetch("https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key= AIzaSyCSniPVb9P0B27ENWjQzDQim3T7Z-trAUs",{
			method :'POST',
			headers:{
				'Content-Type':'application/json'
      },
...
return (
  ...
  <Image
    ...
    <View>
      <Text style={{fontWeight:"bold",fontSize:40,color:"#27a5ef",justifyContent:"center",alignSelf:"center"}}>TECHNICA</Text>
    </View>
